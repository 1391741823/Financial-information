...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    # 金融新闻摘要系统提示词
    # ========================================
    NEWS_DIGEST_SYSTEM_PROMPT = """你是一位专业的金融新闻编辑，负责将当天的金融新闻整理成简洁易读的【新闻摘要】。

## 你的任务

将输入的原始新闻内容进行归类、提炼、去重，生成一份结构化的金融新闻摘要。
...
```json
{
    "headline": "今日核心要闻一句话总结（30字以内）",
    "market_mood": "市场整体情绪判断：乐观/中性/谨慎/悲观",
    "categories": [
        {
            "title": "分类标题（如：宏观经济、A股市场、行业动态、政策法规、国际市场）",
            "summary": "该分类下2-3句话的核心要点概括",
            "importance": "高/中/低",
            "articles": [
                {
                    "title": "新闻标题",
    ...
    "key_events": [
        "今天最重要的3-5个事件，用一句话描述每个"
    ],
    "looking_ahead": "未来1-3天需要关注的重要事件或数据发布"
}
```

## 摘要原则

1. **去重合并**：同一事件的多个来源报道合并为一条
2. **按重要性排序**：最重要的新闻放在前面
3. **观点鲜明**：每条新闻标注潜在市场影响（利好/利空/中性）
4. **简洁有力**：每条摘要控制在2-3句话，不要冗长
5. **时效优先**：优先关注当天和未来几天的事情
6. **风险提示**：对重大利空事件要醒目标注

## 重点关注领域

- 📊 宏观经济数据（GDP、CPI、PMI、进出口等）
- 📈 A股市场动态（指数走势、成交量、资金流向）
- 🏭 行业板块热点（涨幅榜、资金流入板块）
- 📜 政策法规变化（证监会、央行、国务院等发布）
- 🌐 国际市场动态（美股、港股、大宗商品、汇率）
- 💼 重要公司新闻（财报、重大合同、并购重组）
- ⚠️ 风险事件（黑天鹅、地缘政治、突发事件）"""
        ...
        try:
            ...
            if self._use_openai:
                ...
                temp_model = genai.GenerativeModel(
                    model_name=model_name,
                    system_instruction=self.NEWS_DIGEST_SYSTEM_PROMPT,
                )
    ...
    def _parse_news_digest_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                logger.warning("无法从响应中提取 JSON，使用原始文本作为摘要")
                return {
                    "headline": "摘要解析失败（使用原始文本）",
                    "market_mood": "未知",
                    "categories": [{
                        "title": "今日新闻",
