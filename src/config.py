...
class Config:
    ...
    # === 金融新闻摘要配置 ===
    news_topics: List[str] = field(default_factory=lambda: ["宏观经济", "A股市场", "行业动态", "国际市场", "政策法规"])
    news_max_articles: int = 5  # 每个话题最大抓取文章数
    news_digest_schedule: str = "evening"  # morning / evening / both
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # 解析新闻摘要配置
        news_topics_str = os.getenv('NEWS_TOPICS', '宏观经济,A股市场,行业动态,国际市场,政策法规')
        news_topics = [t.strip() for t in news_topics_str.split(',') if t.strip()]

        return cls(
            news_topics=news_topics,
            news_max_articles=int(os.getenv('NEWS_MAX_ARTICLES', '5')),
            news_digest_schedule=os.getenv('NEWS_DIGEST_SCHEDULE', 'evening'),
